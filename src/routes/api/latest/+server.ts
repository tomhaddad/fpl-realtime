...
const fetchLeagueData = async () => {
  ...
  try {
    const response = await fetch(import.meta.env.VITE_FPL_API_HOST + 'bootstrap-static/');
    const data = await response.json();
    const currentEvent = data.events.find((event: { is_current: boolean }) => event.is_current);
    gameweek = currentEvent.id;
  } catch (e) {
    console.error('Failed to establish current game week:', e);
  }
};
...
const fetchRealtimeScores = async () => {
  ...
  for (const team of leagueData.standings.results) {
    ...
    let currentPoints = teamData.total - teamData.event_total || 0; // subtract the current gameweek points, as they will be calculated below
    try {
      // todo: cache this
      const currentTeamResponse = await fetch(
        import.meta.env.VITE_FPL_API_HOST + 'entry/' + team.entry + '/event/' + gameweek + '/picks/',
      );
...
export async function fallback({ request }) {
  return text(`${request.method} request not supported on this endpoint!`);
}
